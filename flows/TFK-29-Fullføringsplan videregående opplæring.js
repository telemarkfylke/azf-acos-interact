const description = 'TFK-29 - Fullføringsplan videregående opplæring'
const title = 'Fullføringsplan videregående opplæring'
  ...
  archive: {
    ...
    options: {
      mapper: (flowStatus, base64, attachments) => {
        const informasjonOmEleven = flowStatus.parseJson.result.DialogueInstance.Opplysninger.Informasjon_om_eleven
        const school = schoolInfo.find(school => school.officeLocation === informasjonOmEleven.Skole || school.primaryLocation === informasjonOmEleven.Skole)
        if (!school) throw new Error(`Could not find any school matching: ${informasjonOmEleven.Skole}`)
    ...
    options: {
      mapper: (flowStatus) => {
        return {
          ...
          department: '',
          description,
          type: 'Fullføringsplan videregående opplæring',
