const description = 'TFK-34 - Takker nei til tilbud om særskilt språkopplæring'
const title = 'Takker nei til tilbud om særskilt språkopplæring'
  ...
  archive: {
    ...
    options: {
      mapper: (flowStatus, base64, attachments) => {
        const gruppe = flowStatus.parseJson.result.DialogueInstance.Tilbakemelding_pa_vedtak.Gruppe
        const school = schoolInfo.find(school => school.officeLocation === gruppe.Skole || school.primaryLocation === gruppe.Skole)
        if (!school) throw new Error(`Could not find any school matching: ${gruppe.Skole}`)
    ...
    options: {
      mapper: (flowStatus) => {
        return {
          ...
          department: '',
          description,
          type: 'Takker nei til tilbud om særskilt språkopplæring',
