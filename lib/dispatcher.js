...
// Process a single blob by path (for Blob Trigger)
const processSingleBlob = async (blobPath) => {
  logger.logConfig({ prefix: 'azf-acos-interact - processSingleBlob' })
  // Find which flow this blob belongs to
  const filename = blobPath.split('/')[0] // expects acosId as first folder
  const schemaNames = readdirSync('./flows').map(f => mapFlowFile(f))
  const flowDef = schemaNames.find(s => s.acosId === filename)
  if (!flowDef) {
    logger.error('No flow definition found for blob path {BlobPath}', blobPath)
    return
  }
  let file
  try {
    file = require(flowDef.filepath)
  } catch (error) {
    logger.errorException(error, 'Could not require schema flow file {SchemaFilepath}', flowDef.filepath)
    return
  }
  if (!file.config.enabled) {
    logger.warn('Flow is not enabled for {AcosId}', flowDef.acosId)
    return
  }
  // Get blob info
  const blobOptions = {
  ...
  let blob
  try {
    const blobs = await list(blobPath, blobOptions)
    blob = blobs.find(b => b.path === blobPath)
    if (!blob) {
      logger.error('Blob not found: {BlobPath}', blobPath)
      return
    }
  } catch (error) {
    logger.errorException(error, 'Could not get blob {BlobPath}', blobPath)
    return
  }
  // Minimal flowStatus for single blob
  const flowStatus = {
    createdTimeStamp: new Date(),
    finished: false,
    failed: false,
    refId: blobPath.split('/')[1] || 'unknown',
    acosId: flowDef.acosId,
    acosName: flowDef.acosName,
    blobDir: blobPath.split('/').slice(0, -1).join('/'),
    runs: 0,
    nextRun: new Date().toISOString()
  }
  try {
    await runFlow(file, flowStatus, [blob])
    logger.info('Successfully processed single blob {BlobPath}', blobPath)
  } catch (error) {
    logger.errorException(error, 'Failed to process single blob {BlobPath}', blobPath)
  }
}

module.exports = { dispatcher, mapFlowFile, createRefIdCollections, processSingleBlob }
