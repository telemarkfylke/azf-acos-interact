...
const singleGraphRequest = async (resource, options = { method: 'get', body: {} }) => {
  ...
  logger.info('{method} {beta} {advanced} {resource}', `method: ${method}`, `beta: ${Boolean(beta)}`, `advanced: ${Boolean(advanced)}`, resource.includes('skiptoken=') ? resource.substring(0, resource.indexOf('skiptoken=')) : resource)
  const headers = {
...
const pagedGraphRequest = async (resource, options = {}) => {
  ...
  while (!finished) {
    ...
    while (!ok && retries < retryLimit) {
      try {
        ...
        if (retries === retryLimit) {
          logger.warn('`ÅNEI, nå har vi feilet {retries} ganger` Vi prøver ikke mer...', error.toString())
          throw error
        } else {
          logger.warn('ÅNEI, graph feila Vi prøver en gang til {error}', error.toString())
        }
      }
    }

    logger.info(`Got ${res.value.length} elements from page ${page}, will check for more`)
    ...
    if (onlyFirstPage) {
      logger.info('onlyFirstPage is true, quick returning - enjoying your testing! 😁')
      finished = true
    }
  }
  result.count = result.value.length
  logger.info(`Found a total of ${result.count} elements`)
  return result
}
