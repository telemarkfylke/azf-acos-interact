...
const singleGraphRequest = async (resource, options = { method: 'get', body: {} }) => {
  ...
  logger.info('singleGraphRequest {method} {beta} {advanced} {resource}', `method: ${method}`, `beta: ${Boolean(beta)}`, `advanced: ${Boolean(advanced)}`, resource.includes('skiptoken=') ? resource.substring(0, resource.indexOf('skiptoken=')) : resource)
  const headers = {
