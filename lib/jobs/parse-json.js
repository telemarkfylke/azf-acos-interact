...
module.exports = async (blobs, flowStatus, jobDef) => {
  ...
  let result = {}
  {
    logger.info('Running function - avlevert datafil')
    const excludeBlobs = []
    ...
    if (!jobDef.options?.mapper) {
      logger.info('No mapper function defined')
      throw new Error('Missing mapper function in job definition - required (for now)')
    }
    if (jobDef.options?.mapper) {
      logger.info('Running function - mapper')
      result.mapped = jobDef.options.mapper(jsonData)
    }
    result.jsonFile = jsonBlob
    logger.info('Got data from avlevert datafil: {fileName}', jsonBlob.name)

    logger.info('Finding files and metadata for delivered files')
    ...
    */
    ...
    result.files = files
    logger.info('Finished finding metadata for delivered files')
  }
