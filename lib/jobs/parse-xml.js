...
module.exports = async (blobs, flowStatus) => {
  ...
  let result = {}
  {
    logger.info('Running function - egendefinert avleveringsfil')
    ...
    logger.info('Finished - egendefinert avleveringsfil: {fileName}', xmlBlob.name)
  }
  {
    logger.info('Running function - finding websak_hode.xml or {Skjemanavn}{_flowStatus.refId}.xml', flowStatus.acosName, flowStatus.refId)
    ...
    const files = filteredFiles.map(file => {
      ...
      // Sjekker om BESK === name, dersom det er det så skal vi bruke BESK som descString, ellers skal vi bruke BESK + filtype.
      const descString = (file.$.BESK === name) ? file.$.BESK : `${file.$.BESK}.${name.split('.').pop().replace(/(\.[^.]+)$\1/g, '')}`
    ...
    result.files = files
    logger.info('Finished - websak_hode.xml or {Skjemanavn}{_flowStatus.refId}.xml: {fileName}', flowStatus.acosName, flowStatus.refId, xmlBlob.name)
  }
