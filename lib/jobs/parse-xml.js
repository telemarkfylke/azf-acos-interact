...
module.exports = async (blobs, flowStatus) => {
  ...
  }
  {
    ...
    const files = filteredFiles.map(file => {
      const name = file.$.TYPE === 'H' ? 'Skjema.pdf' : file._.replace(/[^a-zA-Z0-9æøåÆØÅ ._\/-]/gi, '_').replace('__', '_').trim() // eslint-disable-line no-useless-escape
      const descString = file.$.BESK === name ? file.$.BESK : `${file.$.BESK}.${name.split('.').pop()}`
      const desc = file.$.TYPE === 'H' ? 'Skjema.pdf' : descString.replace(/[^a-zA-Z0-9æøåÆØÅ ._\/-]/gi, '_').replace('__', '_').trim() // eslint-disable-line no-useless-escape
      let path
      if (file.$.TYPE === 'H') {
