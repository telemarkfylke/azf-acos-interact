...
module.exports = async (jobDef, flowStatus) => {
  ...
  if (flowStatus.sharepointList.retryPayload) {
    logger.info('Retry payload is present. Will use it')
  ...
  } else logger.info('payload.request is empty. Moving on to file uploads')

  const failedFileUploads = []
  for (const fileUpload of fileUploads) {
    try {
      logger.info('Starting file upload to SharePoint. fileName: {fileName}, fileDesc: {fileDesc}, siteName: {siteName}', fileUpload.file.name, fileUpload.file.desc, fileUpload.siteName)
      ...
      logger.info('Successfully uploaded file to SharePoint. fileName: {fileName}, fileDesc: {fileDesc}, legalFilename: {legalFilename}, siteName: {siteName}', fileUpload.file.name, fileUpload.file.desc, legalFilename, fileUpload.siteName)
    } catch (error) {
      failedFileUploads.push(fileUpload)
      const errorMsg = error.response?.data || error.stack || error.toString()
      logger.errorException(error, 'Failed when uploading file to SharePoint. fileName: {fileName}, fileDesc: {fileDesc}, siteName: {siteName}. Error: {@message}', fileUpload.file.name, fileUpload.file.desc, fileUpload.siteName, errorMsg)
    }
  }
  ...
  logger.info('Successfully created {listItemCount} list items', payload.requests.length)
