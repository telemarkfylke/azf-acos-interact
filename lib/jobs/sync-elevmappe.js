...
module.exports = async (jobDef, flowStatus) => {
  ...
  if (personData.fakeSsn) {
    logger.info('syncing elevmappe with fake SSN')
  ...
  if (personData.manualData && !personData.fakeSsn) {
    logger.info('syncing elevmappe with skipDSF')
  ...
  logger.info('Successfully synced elevmappe privatePerson: {recNo}, elevmappe: {caseNumber}', data.privatePerson.recno, data.elevmappe.CaseNumber)
  return data
}
